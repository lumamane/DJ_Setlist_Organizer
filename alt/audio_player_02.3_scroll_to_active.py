...
class AudioPlayer(QMainWindow):
    ...
    def scroll_to_playing(self):
        ...
        if self.current_playing is None or self.current_playing not in self.visual_order:
            return
        row = self.visual_order.index(self.current_playing)
        self.file_list.setCurrentRow(row)
        self.file_list.scrollToItem(self.file_list.item(row), QListWidget.ScrollHint.EnsureVisible)
        self.reset_highlights()
        self.file_list.item(row).setBackground(QColor("#2a4a2a"))
