...
class AudioPlayer(QMainWindow):
    ...
    def setup_ui(self):
        ...
        # --- Folder Info ---
        self.folder_info = QLabel("Folder: | Total: 00:00:00 | Remaining: 00:00:00 | Finish: 00:00")
        ...
        self.play_btn.setStyleSheet("""
            QPushButton { background-color: #2196F3; color: white; font-weight: bold; padding: 8px; border-radius: 4px; }
        ...
        self.prev_btn.setStyleSheet("""
            QPushButton { background-color: #555; color: white; font-weight: bold; padding: 8px; border-radius: 4px; }
        ...
        self.scroll_to_playing_btn.setStyleSheet("""
            QPushButton { background-color: #9C27B0; color: white; font-weight: bold; padding: 8px; border-radius: 4px; }
    ...
    def _play_track(self, index):
        ...
        self.track_info = QLabel(os.path.basename(self.playlist[index]))
    ...
    def play_pause(self):
        if self.player.is_playing():
            ...
            if hasattr(self, 'track_info') and self.current_playing:
    ...
    def update_progress(self):
        if self.player.get_media():
            ...
            if self.current_index >= 0:
                ...
                finish_time = (datetime.datetime.now() + datetime.timedelta(seconds=remaining_seconds)).strftime("%H:%M")
                self.folder_info.setText(
                    f"Folder: {os.path.basename(self.current_folder) if self.current_folder else 'No folder selected'} | "
                    f"Total: {total_h:02d}:{total_m:02d}:{total_s:02d} | "
                    f"Remaining: {remaining_h:02d}:{remaining_m:02d}:{remaining_s:02d} | "
                    f"Finish: {finish_time}"
                )
