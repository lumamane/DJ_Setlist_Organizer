...
from PyQt6.QtWidgets import (
    ...
    QMessageBox, QSlider, QInputDialog, QSizePolicy
)
...
class AudioPlayer(QMainWindow):
    ...
    def setup_shortcuts(self):
        # Playback
        QShortcut(QKeySequence(Qt.Key.Key_Space), self, activated=self.play_pause)
        ...
        QShortcut(QKeySequence("Ctrl+Up"), self, activated=lambda: self.set_volume(min(100, self.volume_slider.value() + 5)))
        QShortcut(QKeySequence("Ctrl+Down"), self, activated=lambda: self.set_volume(max(0, self.volume_slider.value() - 5)))
        QShortcut(QKeySequence("Ctrl+Shift+Right"), self, activated=lambda: self.seek_forward(5))
        QShortcut(QKeySequence("Ctrl+Shift+Left"), self, activated=lambda: self.seek_backward(5))

        # Playlist
        QShortcut(QKeySequence("Ctrl+O"), self, activated=self.select_folder)
        QShortcut(QKeySequence(Qt.Key.Key_F5), self, activated=self.refresh_songlist)
        ...
        QShortcut(QKeySequence(Qt.Key.Key_F2), self, activated=self.rename_selected_file)
        QShortcut(QKeySequence("Ctrl+R"), self, activated=self.rename_all)
                ...
                for file in files:
                    if file.lower().endswith(('.mp3', '.m4a', '.wav')):
                        ...
                        try:
                            ...
                            logger.error("Failed to parse media file %s: %s", path, e)
                            continue
        except Exception as e:
            logger.error("Failed to scan folder %s: %s", folder, e)
            QMessageBox.warning(self, "Error", f"Failed to scan folder: {e}")
            return
        self.playlist.sort()
