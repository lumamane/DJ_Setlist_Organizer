...
from PyQt6.QtWidgets import (
    ...
    QMessageBox, QSlider, QInputDialog, QSizePolicy, QProgressDialog, QDialog,
    QTextEdit, QGroupBox
)
...
class AudioPlayer(QMainWindow):
    ...
    logger.debug("on_media_end event received")
...
def setup_ui(self):
    # Central widget and main layout (horizontal)
    ...
    # --- LEFT PANEL: File & Playlist Management ---
    left_panel = QWidget()
    left_panel.setFixedWidth(300)
    left_panel.setStyleSheet("background-color: #1e1e1e; border-radius: 5px;")
    left_layout = QVBoxLayout(left_panel)
    left_layout.setContentsMargins(5, 5, 5, 5)
    left_layout.setSpacing(5)

    # --- Folder & Playlist Group ---
    folder_group = QGroupBox("Folder & Playlist")
    folder_layout = QHBoxLayout(folder_group)
    self.folder_btn = QPushButton("Select Folder")
    self.folder_btn.clicked.connect(self.select_folder)
    self.folder_btn.setStyleSheet("background-color: #4CAF50; color: white; font-weight: bold; padding: 6px; border: none; border-radius: 5px;")
    self.refresh_btn = QPushButton("Refresh")
    self.refresh_btn.clicked.connect(self.refresh_songlist)
    self.refresh_btn.setStyleSheet("background-color: #FFC107; color: #222; font-weight: bold; padding: 6px; border: none; border-radius: 5px;")
    ...
    self.search_box.setStyleSheet("background-color: #333; color: #eee; border: 1px solid #444; border-radius: 5px; padding: 4px;")
    ...
    self.file_list.setStyleSheet("background-color: #1e1e1e; color: #eee; border: 1px solid #444; border-radius: 5px; padding: 2px;")
    ...
    self.rename_all_btn.setStyleSheet("background-color: #FF9800; color: white; font-weight: bold; padding: 6px; border: none; border-radius: 5px;")
    ...
    self.start_index_input.setStyleSheet("background-color: #333; color: #eee; border: 1px solid #444; border-radius: 5px; padding: 4px;")
    self.rename_one_btn = QPushButton("Rename Selected")
    self.rename_one_btn.clicked.connect(self.rename_selected_file)
    self.rename_one_btn.setStyleSheet("background-color: #607D8B; color: white; font-weight: bold; padding: 6px; border: none; border-radius: 5px;")
    self.delete_btn = QPushButton("Delete Selected")
    self.delete_btn.clicked.connect(self.delete_selected_file)
    self.delete_btn.setStyleSheet("background-color: #F44336; color: white; font-weight: bold; padding: 6px; border: none; border-radius: 5px;")
    rename_layout.addWidget(self.rename_all_btn)
    rename_layout.addWidget(self.start_index_input)
    rename_layout.addWidget(self.rename_one_btn)
    rename_layout.addWidget(self.delete_btn)
    left_layout.addLayout(rename_layout)
    ...
    self.undo_btn.setStyleSheet("background-color: #607D8B; color: white; font-weight: bold; padding: 6px; border: none; border-radius: 5px;")
    self.redo_btn = QPushButton("Redo")
    self.redo_btn.clicked.connect(self.redo_playlist_reorder)
    self.redo_btn.setStyleSheet("background-color: #607D8B; color: white; font-weight: bold; padding: 6px; border: none; border-radius: 5px;")
    undo_layout.addWidget(self.undo_btn)
    undo_layout.addWidget(self.redo_btn)
    left_layout.addWidget(undo_group)

    # --- RIGHT PANEL: Playback & Now Playing ---
    right_panel = QWidget()
    right_panel.setStyleSheet("background-color: #2b2b2b; border-radius: 5px;")
    ...
    self.now_playing_label.setStyleSheet("color: #eee; font-weight: bold; font-family: monospace; font-size: 14px;")
    now_playing_layout.addWidget(self.now_playing_label)
    self.folder_info = QLabel("AudioSetlist | Songs: 0 | Total: 00:00:00 | Remaining: 00:00:00 | Finish: 00:00 | Folder: ")
    self.folder_info.setStyleSheet("color: #eee; font-weight: bold; font-family: monospace;")
    now_playing_layout.addWidget(self.folder_info)
    right_layout.addWidget(now_playing_group)
    ...
    self.prev_btn.setStyleSheet("background-color: #757575; color: white; font-weight: bold; padding: 10px; border: none; border-radius: 5px;")
    self.play_btn = QPushButton("▶/⏸")
    self.play_btn.clicked.connect(self.play_pause)
    self.play_btn.setStyleSheet("background-color: #2196F3; color: white; font-weight: bold; padding: 10px; border: none; border-radius: 5px;")
    self.stop_btn = QPushButton("◼")
    self.stop_btn.clicked.connect(self.stop)
    self.stop_btn.setStyleSheet("background-color: #F44336; color: white; font-weight: bold; padding: 10px; border: none; border-radius: 5px;")
    self.next_btn = QPushButton(">>")
    self.next_btn.clicked.connect(self.next_track)
    self.next_btn.setStyleSheet("background-color: #757575; color: white; font-weight: bold; padding: 10px; border: none; border-radius: 5px;")
    ...
    settings_group = QGroupBox("Volume & Settings")
    ...
    self.loop_btn.setStyleSheet("background-color: #9C27B0; color: white; font-weight: bold; padding: 6px; border: none; border-radius: 5px;")
    loop_layout.addWidget(self.loop_btn)
    self.help_btn = QPushButton("Help (F1)")
    self.help_btn.clicked.connect(self.show_help)
    self.help_btn.setStyleSheet("background-color: #607D8B; color: white; font-weight: bold; padding: 6px; border: none; border-radius: 5px;")
    loop_layout.addWidget(self.help_btn)
