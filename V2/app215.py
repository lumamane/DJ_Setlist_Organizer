...
class DJApp(QMainWindow):
    ...
    def init_ui(self):
        ...
        self.song_table.setEditTriggers(QTableView.EditTrigger.NoEditTriggers)
        ...
        bottom_controls.addWidget(QLabel("Rename - Start Index:"))
    ...
    def table_key_press_event(self, event):
        if event.key() in (Qt.Key.Key_Return, Qt.Key.Key_Enter):
            ...
            super(QTableView, self.song_table).keyPressEvent(event)

    def keyPressEvent(self, event):
        if event.key() == Qt.Key.Key_Space:
            if self.current_playing is not None:
                self.song_table.selectRow(self.current_playing)
                self.song_table.scrollTo(self.song_model.index(self.current_playing, 0))
        else:
            super().keyPressEvent(event)

    def highlight_playing_row(self, row):
        """Highlight the currently playing row in blue."""
        self.song_table.clearSelection()
        self.song_table.selectRow(row)
        self.song_table.setStyleSheet(
            f"QTableView::item:selected {{ background-color: {PALE_MOON_BLUE}; color: black; }}"
        )
    ...
    def on_rows_moved(self, parent, start, end, destination, row):
        ...
        if current_filename:
            ...
            if not matching.empty:
                ...
                self.highlight_playing_row(self.current_playing)
            else:
                self.current_playing = None
    ...
    def play_song(self, row):
        ...
        try:
            ...
            self.highlight_playing_row(row)
            self.song_table.selectRow(row)
    ...
    def stop_playback(self):
        ...
        self.song_table.setStyleSheet("")  # Reset style sheet
